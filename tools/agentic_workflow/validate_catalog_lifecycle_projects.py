#!/usr/bin/env python3
"""Validate the Project lifecycle UI suite in its RED or GREEN phase."""
...
TEST_PATH = "src/features/projects/ProjectsPage.test.tsx"
EXPECTED_RED_TESTS = [
    "describes every Task beneath a Project before archive confirmation",
    "archives active Tasks but preserves an already archived Task timestamp",
    "restores an archived Client and Project while Tasks and siblings stay archived",
    "keeps a persistence error visible and Retry opens a fresh Project preview",
]
...
def expects_red(tasks_text: str) -> bool:
    return bool(re.search(r"^- \[ \] 6\.1\b", tasks_text, re.MULTILINE))
...
def validate_result(returncode: int, output: str, *, red: bool) -> str | None:
    if returncode == 0:
        return None
    if not red:
        return "Project lifecycle UI suite failed after task 6.1"
    failures = FAILED_TEST.findall(output)
    if failures != EXPECTED_RED_TESTS:
        return f"RED evidence must contain exactly the expected Project UI failures; observed {failures!r}"
