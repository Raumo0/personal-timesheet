...
- Inspect the current task-bound diff for scope discipline, task/project/client
  ...
  active-name uniqueness, archival preservation, transaction atomicity, route
  context, accessibility, and backup compatibility wherever applicable.
- Require the receipt-bound implementer report, execution state for
  `manage-tasks-with-inherited-rates`, and fresh
  `.agentic-workflow/validation-evidence.json` produced for the same worktree
  and diff. The evidence must report `overall_status: pass`, match the current
...
- For task 9.1, review integrated coverage across every delta requirement and
  confirm the broader frontend, Rust, OpenSpec, and diff checks recorded in the
  implementer report. Treat any unperformed native Tauri smoke check only as the
  explicit limitation documented in `validation.md`, never as passed evidence.
