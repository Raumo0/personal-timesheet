...
- Review one selected `tasks.md` checkbox at a time against its exact files,
  focused coverage command and its allowed passing or expected-failure result,
  `proposal.md`, all affected delta specs, and the
...
- Inspect the current task-bound diff for directional archive/restore rules,
  ...
  wherever applicable.
- For task 10, require that a rollback failure retains the original apply
  failure and that Project workspace context errors render a Retry path that
  reloads the same route rather than leaving a permanent loading state.
