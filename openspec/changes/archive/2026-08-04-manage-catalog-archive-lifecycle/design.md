...
### Preserve primary persistence failures and recover route loading failures

If applying a plan fails and `ROLLBACK` fails too, surface a lifecycle error
that retains both causes. The UI must not imply that the original write was
rolled back. Project workspace context loading must settle to a visible,
recoverable error with Retry on any catalog read failure rather than remaining
in a loading state; a later successful Retry uses the same route parameters.

Replacing the original error with the rollback error was rejected because it
hides the operation that may have partially persisted. Leaving the route in a
permanent loading state was rejected because it prevents the user from
recovering after a transient local-database failure.

Focused coverage is valid when it passes immediately because the behavior is
already present. A failing result is useful only when it demonstrates a missing,
specified behavior; it is not itself a task-completion requirement.
