...
- [x] 1.1 RED: add a focused acceptance test under `tests/` that creates a
...
- [x] 1.3 Configure `openspec/config.yaml` with context and artifact rules for
...
- [x] 2.1 Update `AGENTS.md` and governed-schema apply instructions so native
  ...
  editing it.
- [x] 2.2 Extend `tests/test_implementation_loop_skill.py` and related
...
- [x] 3.1 Create a narrow non-product pilot change with
...
- [x] 3.2 Execute the pilot's one bounded task through native apply followed by
...
- [x] 3.3 Review the pilot outcome with the user. If it demonstrates the
...
- [x] 4.1 Run `python3 -m unittest discover -s tests -v`,
  `pnpm exec openspec schema validate governed-spec-driven`,
...
- [x] 4.2 Run the authorized independent whole-change review through
